  ...
  private def optCacheSecs(annotationo: Option[universe.Annotation]): Int = {
    annotationo match {
      ...
      case Some(annotation) =>
        ...
        if (tpeString == typeOfCacheActionDay)
          -annotation.tree.children.tail(0).toString.toInt * 24 * 60 * 60
        else if (tpeString == typeOfCacheActionHour)
          -annotation.tree.children.tail(0).toString.toInt      * 60 * 60
        else if (tpeString == typeOfCacheActionMinute)
          -annotation.tree.children.tail(0).toString.toInt           * 60
        else if (tpeString == typeOfCacheActionSecond)
          -annotation.tree.children.tail(0).toString.toInt
        else if (tpeString == typeOfCachePageDay)
          annotation.tree.children.tail(0).toString.toInt * 24 * 60 * 60
        else if (tpeString == typeOfCachePageHour)
          annotation.tree.children.tail(0).toString.toInt      * 60 * 60
        else if (tpeString == typeOfCachePageMinute)
          annotation.tree.children.tail(0).toString.toInt           * 60
        else if (tpeString == typeOfCachePageSecond)
          annotation.tree.children.tail(0).toString.toInt
        else
          0
    }
